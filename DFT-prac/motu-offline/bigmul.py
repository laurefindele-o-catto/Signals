...
def from_limbs(sign, limbs, base_digits=BASE_DIGITS):
    ...
    for limb in limbs :
        total = limb + carry 
        formatted_digits.append(total % base)
        carry = total // base

    while carry > 0 : 
        formatted_digits.append(carry % base)
        carry = carry //base

    while len(formatted_digits) > 1 and formatted_digits[-1]==0 :
        formatted_digits.pop()

    if len(formatted_digits)==1 and formatted_digits[0]==0 :
        return "0"

    msb = str(formatted_digits.pop())

    str_list = []
    for limb in reversed(formatted_digits) : 
        text = str(limb).zfill(base_digits)
...
def multiply_transform(a, b, engine):
    ...
    A = engine.transform(a_new)
    B = engine.transform(b_new)
    res = A * B 
    result = engine.inverse(res)
    return np.round(result.real).astype(np.int64) , N
...
def multiply_schoolbook(a, b):
    ...
    Na, Nb = len(a), len(b)
    result = np.zeros(Na + Nb - 1, dtype=np.int64)
    for i in range(Na):
        result[i:i + Nb] += a[i] * b
    return result
...
def multiply(text_a, text_b, method):
    ...
    N = None
    if method == "dft":
        result, N = multiply_transform(limbs_a, limbs_b, DFTAnalyzer())
    elif method == "fft":
        result, N = multiply_transform(limbs_a, limbs_b, FFTTransformer())
    elif method == "arbitrary":
        result, N = multiply_transform(limbs_a, limbs_b, ArbitraryLengthFFT())
    elif method == "schoolbook":
        result = multiply_schoolbook(limbs_a, limbs_b)

    product_str = from_limbs(sign, result)
...
def run_single(path, method, out_dir):
    ...
    report_lines = [
        ...
        field("method", method),
        field("digits of A / B", f"{digits_a} / {digits_b}"),
        field("base", f"10^{BASE_DIGITS}"),
        field("limbs of A / B", f"{len(limbs_a)} / {len(limbs_b)}"),
        field("transform length N", N),
        field("digits of product", digits_product),
        field("verification", verdict),
    ]
    write_report(os.path.join(out_dir, "report.txt"), report_lines)

    # if verdict == "MISMATCH":
    #     raise AssertionError(f"MISMATCH on {path} (method={method}) -- see {out_dir}/report.txt")
