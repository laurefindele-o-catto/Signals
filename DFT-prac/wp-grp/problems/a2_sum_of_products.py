...
def choose_transform_length(len_a, len_b, len_c, len_d, engine):
    ...
    # need = max(...)
    # return
    raise NotImplementedError("TODO 1: one length that holds both products")
...
def sum_of_products_transform(a, b, c, d, engine):
    ...
    # need =
    # N =
    # A, B, C, D = spectra of the four padded arrays
    # coeffs = engine.inverse(...).real[:need]
    # return np.rint(coeffs).astype(np.int64), N
    raise NotImplementedError("TODO 2: A*B + C*D, one inverse")
...
def sum_of_products(text_a, text_b, text_c, text_d, method):
    ...
    #     ...
    # coeffs, N = sum_of_products_transform(..., _make_engine(method))
    # return from_limbs(1, coeffs), N, tuple(len(arr) for arr in limbs)
    raise NotImplementedError("TODO 3: limbs in, result string out")
