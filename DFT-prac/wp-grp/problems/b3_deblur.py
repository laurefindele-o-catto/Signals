...
def wrapped_kernel_spectrum(kernel, shape, engine):
    ...
    # padded[...] = kernel
    # padded = np.roll(padded, (...), axis=(0, 1))
    # return transform_2d(padded, engine)
    raise NotImplementedError("TODO 1: spectrum of the kernel wrapped around the origin")
...
def deblur_plane(blurred, kernel, engine, epsilon=1e-12):
    ...
    # blurred_spectrum = transform_2d(...)
    # kernel_spectrum = wrapped_kernel_spectrum(...)
    # restored = np.zeros_like(blurred_spectrum)
    # reliable =
    # restored[reliable] =
    # return inverse_2d(restored, engine).real
    raise NotImplementedError("TODO 2: divide the spectra safely and invert")
