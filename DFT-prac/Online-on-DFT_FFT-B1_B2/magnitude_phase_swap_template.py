...
# # Get rows (H) and columns (W)
# H, W = spectrum.shape
# result = np.ones((H, W), dtype=np.complex128) # Initialize everything to 1+0j

# for i in range(H):
#     for j in range(W):
#         if magnitude[i, j] > epsilon:
#             result[i, j] = spectrum[i, j] / magnitude[i, j]
#         # No else block needed because it's already initialized to 1!
...
def swap_plane_spectra(plane_a, plane_b, engine):
    ...
    swapped_ab = abs(spectrum_a) * unit_phase(spectrum_b) 
    swapped_ba = abs(spectrum_b) * unit_phase(spectrum_a) 
    ...
    result_ab = inverse_2d(swapped_ab, engine)
    result_ba = inverse_2d(swapped_ba, engine)
    return np.real(result_ab), np.real(result_ba)
...
def swap_images(image_a, image_b, engine):
    ...
    if image_a.ndim == 2:
        ...
        #raise NotImplementedError("TODO 3: process grayscale images")
    if image_a.ndim == 3 and image_a.shape[2] == 3:
        ...
        for c in range(3):
            plane_a = image_a[:,:,c]
            plane_b = image_b[:,:,c]
            swap1, swap2 = swap_plane_spectra(plane_a, plane_b, engine)
            result_ab.append(swap1)
            result_ba.append(swap2)
            
        return np.stack(result_ab, axis=2), np.stack(result_ba, axis=2)
...
#python magnitude_phase_swap_template.py --image-a images/sunset512.png --image-b images/skyline512.png --engine fft --out-dir outputs/lab_phase_swap_student