...
def to_limbs(text, base_digits=BASE_DIGITS):
    ...
    if not text:
        return sign, np.array([0], dtype=np.int64)
    
    limbs = []
    for i in range(len(text), 0, -base_digits):
        #pich theke 0 porjonto 4ta kore nei
        #tahole last theke 4ta 4ta kore nibe
        #until first ta hit kore, jeta actually MSD, so 0 theke i porjonto
        start = max(0, i - base_digits)
        limbs.append(int(text[start:i]))
...
def from_limbs(sign, limbs, base_digits=BASE_DIGITS):
    ...
    limbs_list = [int(x) for x in limbs]
    
    while flag == 0:
        n = len(limbs_list)
        
        if limbs_list[curr] >= base:
            carry = limbs_list[curr] // base
            limbs_list[curr] = limbs_list[curr] % base
            if curr <= n-2:
                limbs_list[curr+1] = limbs_list[curr+1] + carry
            else:
                limbs_list.append(carry)
        
        curr += 1
        if curr == len(limbs_list):
            flag = 1
            
    while len(limbs_list) > 1 and limbs_list[-1] == 0:
        limbs_list.pop()
        
    if len(limbs_list) == 1 and limbs_list[0] == 0:
        return "0"
        
    msd = str(limbs_list[-1])
    #shobar last er ta ase, i.e MSD
    text = msd
    
    reversed_remaining_limbs = reversed(limbs_list[:-1])
    
    for limb in reversed_remaining_limbs:
        chunk = str(limb)
        
        while len(chunk) < base_digits:
            chunk = "0" + chunk
            
        #chunk = str(limb).zfill(base_digits)
        text = text + chunk
...
def multiply_transform(a, b, engine):
    ...
    # np.pad adds zeros to the end (right side) of the little-endian arrays
    #a_padded = np.pad(a, (0, n - len(a)), mode='constant')
        
    conv = engine.inverse(engine.transform(a_list) * engine.transform(b_list))
    
    arr = np.round(np.real(conv)).astype(np.int64)
...
def multiply(text_a, text_b, method):
    ...
    if method == "schoolbook":
        n = 0
        product_limbs = (multiply_schoolbook(limbs_a, limbs_b))
        product = from_limbs(final_sign, product_limbs)
    else:
        product_limbs, n = multiply_transform(limbs_a, limbs_b, engine)
        product = from_limbs(final_sign, product_limbs)
...
def run_single(path, method, out_dir):
    ...
    #number gula ke lines e rakhsi
    
    if len(lines) < 2:
        raise ValueError(f"Input file {path} does not contain two numbers.")
    
    text_a = lines[0]
    text_b = lines[1]
    
    product_string, n, limbs_a, limbs_b = multiply(text_a, text_b, method)
    
    num_a = int(text_a)
    num_b = int(text_b)
    exp_product = str(num_a * num_b)
    
    if product_string == exp_product:
        verdict = "MATCH"
    else:
        verdict = "MISMATCH"
        f = 1
    ...
    digits_product = len(product_string.lstrip('+-'))
    ...
    with open(report_path, "w") as f:
        ...
        f.write(f"digits of A / B     : {digits_a} / {digits_b}\n")
        f.write(f"base                : 10^{BASE_DIGITS}\n")
        f.write(f"limbs of A / B      : {len(limbs_a)} / {len(limbs_b)}\n")
        f.write(f"transform length N  : {n}\n")
        f.write(f"digits of product   : {digits_product}\n")
...
# python bigmul.py inputs/4.txt --engine arbitrary --out-dir outputs/task_a/4_arb
