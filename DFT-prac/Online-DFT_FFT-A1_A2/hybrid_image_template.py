...
def hybrid_plane(low_plane, high_plane, kernel, engine):
    ...
    # full =
    full = np.real(inverse_2d(combined, engine))
...
def hybrid_image(low_image, high_image, kernel, engine):
    ...
    if low_image.ndim == 2:
        ...
        # return
        combined_plane = hybrid_plane(low_image, high_image, kernel, engine)
    ...
    if low_image.ndim == 3 and low_image.shape[2] == 3:
        ...
        planes = []
        for c in range(3):
            low_plane = low_image[:, :, c]
            high_plane = high_image[:, :, c]
            combined_plane = hybrid_plane(low_plane, high_plane, kernel, engine)
            planes.append(combined_plane)
...
#python hybrid_image_template.py --low-image images/sunset512.png --high-image images/skyline512.png --kernel-size 31 --engine fft --out-dir outputs/lab_hybrid_student