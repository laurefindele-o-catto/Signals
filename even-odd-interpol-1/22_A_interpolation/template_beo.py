...
def interpolate_signal(
    ...
    is_close = np.isclose(idx, idx_round, atol=10e-6)
...
def time_scale(
    ...
    x_query = interpolate_signal(t, x, t_query)
    
    out_of_range = (t_query < t[0]) | (t_query > t[-1])
    
    x_query = np.where(out_of_range, np.nan, x_query)
...
def plot_pair(t: np.ndarray, x: np.ndarray, y: np.ndarray, title: str):
    ...
    plt.plot(t, y, linewidth = 1.5, label="y(t)", linestyle = "--")
    plt.xlabel("Time")
