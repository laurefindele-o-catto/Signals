...
def test_linearity(apply_system, x1, x2, a, b):
    ...
    output_1 = apply_system(x1.multiply(a).add(x2.multiply(b)))
    output_2 = apply_system(x1).multiply(a).add(apply_system(x2).multiply(b))
    max_diff = max_absolute_difference(output_1, output_2)
...
def test_time_invariance(apply_system, x, k):
    ...
    output_1 = apply_system(x.shift(k))
    output_2 = apply_system(x).shift(k)
    
    return max_absolute_difference(output_1, output_2)
...
def system_b(input_signal: DiscreteSignal):
    ...
    for k in range(start, end+1):
        output_signal.set_value_at_time(k, (input_signal.get_value_at_time(k))*k)
...
def main():
    ...
    print("Conclusion: System B fails time-invariance")
