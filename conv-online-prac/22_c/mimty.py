...
class LTISystem:
    ...
    def output_super(self, super_signal):
        """
        Takes a SuperSignal object, computes the LTI output for each 
        component, scales them, and accumulates them using linearity/superposition.
        """
        super_output = None
        
        for coeff, signal in super_signal.components:
            signal = signal.multiply(coeff)
            output = self.output(signal)
            
            if super_output is None:
                super_output = output
            else:
                super_output = super_output.add(output)
...
if __name__ == "__main__":
    ...
    y_super = system.output_super(superSignal)
    y_super.plot("Output y[n] via superposition")
    plt.show()
