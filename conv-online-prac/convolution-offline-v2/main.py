...
def make_signal(start_time, end_time, values):
    ...
    for i in range(start_time, end_time+1):
        x.set_value_at_time(i, values[i-start_time])
    return x
...
def impulse_moving_average(length):
    ...
    for i in range(0, length):
        h.set_value_at_time(i, float(1/length))
...
def impulse_weighted_smoothing():
    h = DiscreteSignal(0, 2)
    h.set_value_at_time(0, 0.5)
    h.set_value_at_time(1, 0.3)
